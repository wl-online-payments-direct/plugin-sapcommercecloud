...
<%@ taglib prefix="paymentproduct"
           tagdir="/WEB-INF/tags/addons/worldlinegopayb2bcheckoutaddon/responsive/paymentproducts" %>
...
<c:choose>
    ...
    <c:when test="${paymentProduct.id==-2}">
        <div class="worldline_payment_product" title="${groupedCardsLabel}">
            <paymentproduct:hostedCheckoutDetail paymentProductId="${paymentProduct.id}" paymentInfo="${paymentInfo}"
                                                 savedPaymentInfos="${savedPaymentInfos}"/>
        </div>
        <form:input type="hidden" path="savedCardCode"/>
    </c:when>
    <c:otherwise>
        <div id="worldline_payment_product_${paymentProduct.id}"
             class="worldline_payment_product js-worldline_payment_product"
