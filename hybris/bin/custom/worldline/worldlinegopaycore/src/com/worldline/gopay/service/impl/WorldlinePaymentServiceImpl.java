...
import com.worldline.gopay.util.WorldlineLogUtils;
...
public class WorldlinePaymentServiceImpl implements WorldlinePaymentService {
    ...
    protected Converter<com.worldline.gopay.order.data.BrowserData, CustomerDevice> worldlineBrowserCustomerDeviceConverter;
    ...
    public CreatePaymentResponse createPaymentForHostedTokenization(OrderModel orderForCode, WorldlineHostedTokenizationData worldlineHostedTokenizationData) throws WorldlineNonAuthorizedPaymentException {
        ...
        try {
            ...
            throw new WorldlineNonAuthorizedPaymentException(WorldlinegopaycoreConstants.UNAUTHORIZED_REASON.REJECTED);
        } catch (Exception e) {
            logCreatePaymentFailure(LOGGER, "createPaymentForHostedTokenization", params, e);
    ...
    @Override
    @SuppressWarnings("all")
    public CreateHostedCheckoutResponse createHostedCheckout(OrderModel orderForCode, com.worldline.gopay.order.data.BrowserData browserData) {
    ...
    public CreatePaymentResponse createSubsequentPayment(AbstractOrderModel abstractOrderModel) throws WorldlineNonAuthorizedPaymentException {
        ...
        if (!(abstractOrderModel.getPaymentInfo() instanceof WorldlinePaymentInfoModel paymentInfo)
                || (!StringUtils.equals(WorldlinegopaycoreConstants.PAYMENT_METHOD_TYPE.CARD.getValue(), paymentInfo.getPaymentMethod())
    ...
    public Long getNonCapturedAmount(String storeId, String paymentId, CapturesResponse capturesResponse, BigDecimal plannedAmount, String currencyISOcode) {
        ...
        if (WorldlinegopaycoreConstants.PAYMENT_STATUS_ENUM.valueOf(paymentResponse.getStatus()).equals(WorldlinegopaycoreConstants.PAYMENT_STATUS_ENUM.CAPTURED)) {
        ...
        for (Capture capture : capturesResponse.getCaptures()) {
            if (WorldlinegopaycoreConstants.PAYMENT_STATUS_ENUM.CAPTURED.getValue().equals(capture.getStatus()) ||
                    WorldlinegopaycoreConstants.PAYMENT_STATUS_ENUM.CAPTURE_REQUESTED.getValue().equals(capture.getStatus())) {
    ...
    public void setWorldlineBrowserCustomerDeviceConverter(Converter<com.worldline.gopay.order.data.BrowserData, CustomerDevice> worldlineBrowserCustomerDeviceConverter) {
