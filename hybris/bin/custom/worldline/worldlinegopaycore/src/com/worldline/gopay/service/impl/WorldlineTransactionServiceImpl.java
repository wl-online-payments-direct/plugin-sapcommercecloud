...
public class WorldlineTransactionServiceImpl implements WorldlineTransactionService {
    ...
    public void processPaymentLinkEvent(WebhooksEvent webhooksEvent) {
        ...
        final WorldlinegopaycoreConstants.WEBHOOK_TYPE_ENUM webhookType = WorldlinegopaycoreConstants.WEBHOOK_TYPE_ENUM.fromString(webhooksEvent.getType());
        if (webhookType == WorldlinegopaycoreConstants.WEBHOOK_TYPE_ENUM.PAYMENT_LINK_CANCELLED
                || webhookType == WorldlinegopaycoreConstants.WEBHOOK_TYPE_ENUM.PAYMENT_LINK_EXPIRED) {
            if (StringUtils.isBlank(paymentInfo.getPaymentLinkPaymentId())) {
                order.setPaymentStatus(PaymentStatus.WORLDLINE_CANCELED);
                modelService.save(order);
                worldlineBusinessProcessService.triggerOrderProcessEvent(order, WorldlinegopaycoreConstants.WORLDLINE_EVENT_PAYMENT);
            }
        }
    ...
    private PaymentTransactionModel createPaymentTransaction(
        ...
        paymentTransactionModel.setPaymentProvider(WorldlinegopaycoreConstants.PAYMENT_PROVIDER);
    ...
    private void setOrderPaymentStatus(PaymentTransactionModel paymentTransactionModel, PaymentTransactionEntryModel transactionEntryModel) {
        ...
        switch (transactionEntryModel.getType()) {
            case AUTHORIZATION:
                if (WorldlinegopaycoreConstants.PAYMENT_STATUS_CATEGORY_ENUM.SUCCESSFUL.getValue().equals(transactionStatus)) {
                    order.setPaymentStatus(PaymentStatus.WORLDLINE_AUTHORIZED);
                } else if (WorldlinegopaycoreConstants.PAYMENT_STATUS_CATEGORY_ENUM.REJECTED.getValue().equals(transactionStatus)) {
            ...
            case CAPTURE:
                ...
                for (PaymentTransactionEntryModel entry : getCapturePaymentTransactionEntries(paymentTransactionModel)) {
                    if (WorldlinegopaycoreConstants.PAYMENT_STATUS_CATEGORY_ENUM.REJECTED.getValue().equals(transactionStatus)) {
                        order.setPaymentStatus(PaymentStatus.WORLDLINE_REJECTED);
                        modelService.save(order);
                        return;
                    } else if (WorldlinegopaycoreConstants.PAYMENT_STATUS_CATEGORY_ENUM.SUCCESSFUL.getValue().equals(transactionStatus)) {
    ...
    private void resumerProcess(AbstractOrderModel order, PaymentTransactionType transactionType) {
        switch (transactionType){
            ...
            case REFUND_FOLLOW_ON:
                worldlineBusinessProcessService.triggerReturnProcessEvent((OrderModel) order, WorldlinegopaycoreConstants.WORLDLINE_EVENT_REFUND);
                break;
        }
    }
    ...
    private String getTransactionStatus(String status) {
        switch (WorldlinegopaycoreConstants.PAYMENT_STATUS_ENUM.valueOf(status)) {
            case CREATED:
            case REJECTED:
            case REJECTED_CAPTURE:
            case CANCELLED:
                return WorldlinegopaycoreConstants.PAYMENT_STATUS_CATEGORY_ENUM.REJECTED.getValue();
            case REDIRECTED:
            case REFUND_REQUESTED:
            case AUTHORIZATION_REQUESTED:
            case CAPTURE_REQUESTED:
                return WorldlinegopaycoreConstants.PAYMENT_STATUS_CATEGORY_ENUM.STATUS_UNKNOWN.getValue();
            case PENDING_PAYMENT:
            case PENDING_COMPLETION:
            case PENDING_CAPTURE:
            case CAPTURED:
            case REFUNDED:
                return WorldlinegopaycoreConstants.PAYMENT_STATUS_CATEGORY_ENUM.SUCCESSFUL.getValue();
            default:
